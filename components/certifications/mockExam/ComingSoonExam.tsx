...
import { ChevronLeft, MonitorCheck, Clock3 } from "lucide-react";
import { getCompanyIcon } from "@/lib/vendorIcons";
import type { Company, Certification } from "@/lib/companiesData";
import { useLocale } from "@/components/LocaleProvider";

/**
 * Placeholder shown for every company/cert's Exam Simulation page. Same
 * layout/tone as ComingSoonLab/ComingSoonPractice/ComingSoonLearn for
 * consistency. Deliberately unconditional (not gated by hasPracticeBank)
 * per the owner's explicit decision to keep Exam Simulation locked
 * everywhere for now, independent of which certs have a real practice
 * question bank.
 */
export default function ComingSoonExam({ company, cert }: { company: Company; cert: Certification }) {
  const { t } = useLocale();
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 text-center md:px-8">
      ...
      <div className="rounded-2xl border border-border-soft bg-panel p-10">
        ...
        <p className="mx-auto mb-6 max-w-md text-sm text-text-muted">
          {t("mockExam.simInProgress").split("{company}")[0]}
          <strong className="text-text">{company.name}</strong>
          {t("mockExam.simInProgress").split("{company}")[1]}
        </p>
