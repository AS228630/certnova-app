import type { Metadata } from "next";
import Link from "next/link";
import RegisterTriggerLink from "@/components/registration/RegisterTriggerLink";
import { ArrowRight, BookOpen, ShieldCheck, Sparkles, Route } from "lucide-react";

export const metadata: Metadata = {
  title: "IT-сертификации, языковые курсы и коучинг с ИИ | CertCoach",
  description:
    "Готовься к сертификациям AWS, Microsoft, Cisco и другим вместе с CertCoach. Практические лабораторные работы, тренировочные вопросы, ИИ-коуч и структурированные учебные пути — начни бесплатно.",
  alternates: {
    canonical: "https://www.certcoach.de/ru",
    languages: {
      de: "https://www.certcoach.de",
      en: "https://www.certcoach.de/en",
      fa: "https://www.certcoach.de/fa",
      ar: "https://www.certcoach.de/ar",
      uk: "https://www.certcoach.de/uk",
      es: "https://www.certcoach.de/es",
      fr: "https://www.certcoach.de/fr",
      ru: "https://www.certcoach.de/ru",
      tr: "https://www.certcoach.de/tr",
      "x-default": "https://www.certcoach.de/en",
    },
  },
};

const FEATURES = [
  { icon: BookOpen, title: "Учись на практике", desc: "Практические лабораторные работы и проекты для реального опыта." },
  { icon: ShieldCheck, title: "Признанные сертификаты", desc: "Международно признанные сертификаты для твоей карьеры." },
  { icon: Sparkles, title: "ИИ-коуч", desc: "Твой личный ИИ-коуч сопровождает тебя во время обучения." },
  { icon: Route, title: "Учебные пути", desc: "Структурированные пути, разработанные для твоего успеха." }
];

export default function RussianLandingPage() {
  return (
    <main dir="ltr" className="min-h-screen bg-[#0a0a12] text-white">
      <div className="mx-auto max-w-5xl px-6 py-20 text-center">
        <span className="mb-6 inline-block rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-xs font-semibold text-white/70">
          Платформа обучения IT №1 в DACH
        </span>
        <h1 className="font-bold text-4xl leading-tight sm:text-5xl">
          Твоё будущее начинается
          <br />
          <span className="text-violet-400">с новых навыков</span>
        </h1>
        <p className="mx-auto mt-6 max-w-xl text-white/70">
          Учись у экспертов, получай признанные сертификаты и строй карьеру, которую хочешь. От Microsoft и AWS до CompTIA и Cisco.
        </p>
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          <RegisterTriggerLink className="flex items-center gap-2 rounded-full bg-violet-600 px-6 py-3 text-sm font-semibold hover:bg-violet-500">
            Начать бесплатно
            <ArrowRight size={16} />
          </RegisterTriggerLink>
          <Link
            href="/zertifizierungen"
            className="rounded-full border border-white/15 px-6 py-3 text-sm font-semibold hover:bg-white/5"
          >
            Посмотреть курсы
          </Link>
        </div>
      </div>

      <div className="mx-auto max-w-5xl px-6 pb-24">
        <h2 className="mb-8 text-center text-2xl font-bold">Почему CertCoach?</h2>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f) => (
            <div key={f.title} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              <f.icon size={22} className="mb-3 text-violet-400" />
              <p className="mb-1.5 text-sm font-bold">{f.title}</p>
              <p className="text-xs text-white/60">{f.desc}</p>
            </div>
          ))}
        </div>
      </div>

      <div className="border-t border-white/10 py-6 text-center text-xs text-white/40">
        <p>© 2026 CertCoach. Сделано с ❤️ в Германии</p>
        <p className="mt-2">
          <Link href="/" className="underline hover:text-white/70">Deutsch</Link>
          {" · "}
          <Link href="/en" className="underline hover:text-white/70">English</Link>
          {" · "}
          <Link href="/fa" className="underline hover:text-white/70">فارسی</Link>
        </p>
      </div>
    </main>
  );
}
